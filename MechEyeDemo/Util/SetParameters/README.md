...
1. Make sure the `SetParameters` folder is in a location with read and write permissions.
2. Run Cmake and set the source and build paths:
   
   | Field                       | Path                    |
   | :----                       | :----                   |
   | Where is the source code    | xxx/SetParameters       |
   | Where to build the binaries | xxx/SetParameters/build |
...
1. Navigate to the `SetParameters` folder. 
   
   ```bash
   cd /opt/mech-mind/mech-eye-sdk/samples/Util/SetParameters/
   ```
