...
int main(int argc, char *argv[]) 
{
    ...
    serverAddress.sin_addr.s_addr = inet_addr("192.168.11.100"); // 使用本地IP地址
    serverAddress.sin_port = htons(9000); // 设置端口号
            ...
            for (const auto& object: objects) 
            {
                ...
                if (object.label == (choice - 1))
                {
                    ...
                    for (const auto& indices : clusters) 
                    {
                        ...
                        // 输入欧拉角（单位：弧度）
                        double phi = -75.788 * 3.14 / 180;  // 绕 Z 轴的旋转角度
                        double theta = 179.491 * 3.14 / 180;  // 绕 Y' 轴的旋转角度
                        double psi = -161.641 * 3.14 / 180;  // 绕 Z'' 轴的旋转角度

                        // 创建Eigen::Quaterniond对象来存储四元数
                        Eigen::Quaterniond quaternion_ota;

                        // 计算四元数
                        quaternion_ota = Eigen::AngleAxisd(phi, Eigen::Vector3d::UnitZ()) *
                                    Eigen::AngleAxisd(theta, Eigen::Vector3d::UnitY()) *
                                    Eigen::AngleAxisd(psi, Eigen::Vector3d::UnitZ()); 
                        std::cout << "四元数: " << quaternion_ota << std::endl;
                        ...
                        std::ostringstream pose_x, pose_y, pose_z0, pose_z, pose_rr, pose_rp, pose_ry;
                        pose_x << std::fixed << std::setprecision(2) << translation(0)*1000;
                        pose_y << std::fixed << std::setprecision(2) << translation(1)*1000;
                        pose_z0 << std::fixed << std::setprecision(2) << translation(2)*1000 + 500; // 过渡点z方向的值
                        ...
                        //  提取结果(发送数据格式存在问题)
                        poseResult = pose_x.str() + "," + pose_y.str() + "," + pose_z.str() + "," + pose_rr.str() + "," + pose_rp.str() + "," + pose_ry.str() ;

                        //  过渡点+目标点
                        // poseResult = pose_x.str() + "," + pose_y.str() + "," + pose_z0.str() + "," + pose_rr.str() + "," + pose_rp.str() + "," + pose_ry.str() + "," + std::to_string(1) pose_x.str() + "," + pose_y.str() + "," + pose_z.str() + "," + pose_rr.str() + "," + pose_rp.str() + "," + pose_ry.str() + "," + std::to_string(1);
