...
int main(int argc, char *argv[]) 
{
    ...
    std::string imgFile = "beer.png";
    saveMap(color, imgFile);
	...
	if (!parseArguments(argc, argv, config, onnxModelPath, imgFile)) 
    {
		return -1;
    }
    ...
    auto image = cv::imread(imgFile);
    if (image.empty()) {
        std::cout << "Error: Unable to read image at path '" << imgFile << "'" << std::endl;
        return -1;
    }
    // Run inference
    ...
    if (!objects.empty()) 
    {
        ...
        for (const auto& object: objects) 
        {
            if (object.label == 1)
            {
                ...
                // 遍历图像的每个像素并读取像素值
                for (int y = object.rect.y; y < object.rect.y + object.rect.height+0; y++) 
                {
                    for (int x = object.rect.x-0; x < object.rect.x + object.rect.width+0; x++) 
                    {
                        // 读取像素值
                        // uchar pixelValue = grayImage.at<uchar>(y, x);
                        // 设置新的像素值，例如将所有像素设置为0（黑色）
    ...
    // sor.setLeafSize(100.0f, 100.0f, 100.0f);  // 设置滤波器体素大小
    ...
    clusterExtractor.setMinClusterSize(2800);     // 设置最小聚类点数(800)
